...
**[최우선 공통 원칙] 사용자가 다음에 뭘 할지 항상 알 수 있어야 한다.**
어떤 응답이든 — 단계 진행, 에러, 대기, 안내 무엇이든 — 방금 무엇을 했고 이어서 무엇을 하면 되는지가 자연스럽게 드러나야 한다.

**단, "지금 한 일 / 다음 할 일" 처럼 딱지를 붙이지 않는다.** 대화하듯 한두 문장으로 자연스럽게 풀어서 말한다.
- 예: "모델을 선택했어요. 이제 환경을 확인해볼게요." / "환경까지 확인됐으니, 다음은 템플릿을 맞춰볼 차례예요."
- 에러도 자연스럽게: "등록 중에 연결이 안 됐어요. .env 의 주소를 다시 확인해주시겠어요?"

사용자가 "이제 뭘 하지?" 라고 느끼게 두지 않는다. 막다른 응답을 만들지 않는다. 라벨/제목 없이 문장으로 안내한다.
...
```
──────────────────────────────────────────────────────────
(미선택)  ▶1.목록 ·2.선택 ·3.생성 ·4.학습 ·5.로컬추론(선택) ·6.등록(선택) ·7.원격추론
──────────────────────────────────────────────────────────
```
...
**[필수 관문] 목록을 보여주기 전에 `.env` 를 반드시 확인한다:**
...
- **[재개] 사용자가 `.env` 를 입력/저장했다고 하면** (예: "입력했어", "저장했어", "다시 해줘"), 즉시 `check_env.py` 를 **다시 실행**한다.
  ...
  - 여전히 `ready:false` 이면 어떤 값이 비었는지 알려주고 다시 안내한다.
  - "입력했다"는 말을 들으면 반드시 재확인한다. 재확인 없이 계속 대기하지 않는다.
...
3. 모델 있음 7단계
   1 모델 목록 확인   2 모델 선택   3 생성   4 학습
   5 로컬 추론(선택)   6 등록(선택)   7 원격 추론
```
...
(0단계 `.env` 환경변수 체크는 이 7단계보다 먼저 통과하는 관문이며, rules/always 로 관리한다.)

| 단계 | 이름 | 설명 | 실행 조건 |
|---|---|---|---|
| 1 | 모델 목록 확인 | data/ 스캔 + Case 판별 (모델만/학습만/둘다) | 진입 시 자동 |
| 2 | 모델 선택 | 번호 선택. Case 3(둘다)이면 기존모델/새학습 물어봄 | 사용자 선택 |
| 3 | 생성 | Case에 맞는 템플릿 복사 + data 넣기 | 사용자 선택 |
| 4 | 학습 | Case 2/3-학습선택: 의존성체크 → train.py 실행 → saved_model 생성. Case 1/3-기존모델: 스킵 | 사용자 선택 |
| 5 | 로컬 추론 | (선택/스킵가능) predict 호출로 등록 전 로컬 확인 | 사용자 선택 |
| 6 | 등록 | (선택/스킵가능) MLflow log_model | 사용자 선택 |
| 7 | 원격 추론 | 등록 후 원격 엔드포인트 테스트 | 사용자 선택 |

- **3 생성**: 템플릿을 복사해 선택 모델과 합친다. model_register.py, aiu_custom/, requirements.txt, input_example.json 등 생성.
- **4 학습**: Case 2(학습코드만)/Case 3-학습선택 시 실행. 의존성 체크 → train.py 실행 → 모델 생성 → saved_model/ 이동. Case 1(모델파일)/Case 3-기존모델 선택 시 스킵.
- **5 로컬 추론**: 선택, 스킵 가능. 등록 전 로컬에서 predict 호출해 모델이 제대로 추론되나 확인.
- **6 등록**: 선택, 스킵 가능. MLflow log_model로 서버에 등록.
- **5, 6번은 같은 선상**: 모델 준비되면 아무거나 먼저 할 수 있고 스킵도 가능.
- **7 원격 추론**: 등록 후 원격 :predict URL로 테스트.
