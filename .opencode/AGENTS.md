...
어떤 응답이든 — 단계 진행, 에러, 대기, 안내 무엇이든 — 끝에 반드시 다음 두 가지를 포함한다:
- **지금 한 일**: 방금 무엇을 처리/확인했는지 한 줄.
- **다음 할 일**: 사용자가 이어서 무엇을 하면 되는지 한 줄 (다음 단계, 또는 입력해야 할 것, 또는 선택지).

사용자가 "이제 뭘 하지?" 라고 느끼게 두지 않는다. 막다른 응답을 만들지 않는다.
(예: 에러가 나도 "무엇이 실패했고, 다음에 무엇을 하면 되는지" 를 반드시 알려준다.)
