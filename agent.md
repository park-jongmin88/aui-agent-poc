...
### "다음" / "계속" / "이어서" 요청
사용자가 "다음", "계속", "이어서", "그 다음은?" 처럼 모호하게 요청하면:
1. 현재 상태(.aiu_state.json)를 확인한다
2. 다음 단계가 무엇인지 설명한다
3. "다음은 ○○ 단계입니다. 진행할까요?" 라고 물어보고 확인 후 실행한다

### 이전 단계로 되돌리기 (재작업)
"다시 학습", "재학습", "다시 검증", "이전 단계로", "처음부터" 등을 요청하면:
- 해당 단계로 상태를 되돌린 뒤(이후 기록 삭제) 재실행한다
- 예: predicted 상태에서 "다시 학습" → trained 미만으로 되돌리고 train 재실행
- common 의 rewind_to(folder, 목표상태) 사용

### 파일 점검 (매 단계 실행 전)
스크립트는 실행 전 파일 상태를 점검한다 (check_files_consistency):
- **run.py 삭제됨** → init부터 다시 (강제)
- **results/ 모델 삭제됨 (local_tested인데)** → validated로 되돌림 (강제)
- **run.py가 검증 이후 수정됨** → "다시 검증을 권장합니다" 안내 (강제 아님)

사용자가 파일을 직접 수정/삭제한 경우, 위 점검 결과를 우선 안내하고
필요한 단계로 돌아가도록 유도한다.
