...
5. **평가(judge)** — 서빙과 분리된 별도 스크립트(`judge_eval.py`)로, 쌓인 trace를 MLflow 정석 방식(`make_judge`)으로 평가한다.
...
judge_eval.py        MLflow make_judge 등록 + trace 평가 스크립트 (서빙과 분리)
...
judge = make_judge(
    name="answer_quality",
    instructions="질문 {{ inputs }} 에 대한 답변 {{ outputs }} 평가 ... 1~5등급",
    model="gateway:/<AI Gateway 엔드포인트명>",
    feedback_value_type=int,    # 5등급 정수
)
judge.register(experiment_id=...)                     # Judges 탭에 등록
...
### 사용
```bash
python judge_eval.py register   # judge 등록 (Judges 탭)
python judge_eval.py evaluate   # 최근 trace 평가
```
