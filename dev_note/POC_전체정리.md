...
- **버전 선택**: 별칭(@production) 의존을 제거하고 버전 번호로 로드한다. client 가 `prompt_id` + `prompt_version` 을 보내면 `load_prompt(name, version=N)` 으로, 버전 생략 시 최신을 로드한다. "프롬프트 선택 → 버전 선택" 2단계. OSS MLflow 는 `search_prompt_versions`(Databricks 전용)가 없어 버전 목록을 load_prompt 순차탐색으로 조회한다. (→ 프롬프트_버전.md)
