---
name: validate
description: "현재 작업 폴더의 run.py가 9-섹션 표준 구조를 따르는지 검증한다. 작업 순서 2단계 (init 후 필수). 다음과 같은 요청에 사용: '검증해줘', '확인해줘', '맞는지 봐줘', '문제없는지 체크해줘', '구조 맞는지 봐줘', '이상없어?', '바로 학습해도 돼?' 영어/추가 표현: '발리데이트', 'validate', 'check', '점검해줘', '검사해줘', '제대로 됐어?', '학습 가능해?', '문제 있어?'."
---
# validate - run.py 검증
