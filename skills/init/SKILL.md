---
name: init
description: "모델 폴더의 기초 자료(데이터, 기존 코드, 모델 파일 등)를 분석해 표준 run.py를 생성한다. 자료가 없으면 template/run.py를 복사해 시작점을 만든다. 초기화, 새 모델 시작, run.py 만들기 요청 시 사용."
---
...
4. template/run.py 의 9-섹션 구조를 기준으로, 자료를 반영한 run.py를 작성한다:
   - 자료가 충분하면 → 섹션을 실제 코드로 채움
   - 자료가 없거나 빈 폴더면 → template 복사 후 TODO 항목 안내
5. 작성/수정 전에는 반드시 사용자에게 내용을 보여주고 동의받는다.
6. 완료 후 validate 실행을 권한다.

## 주의
- template/ 원본은 절대 수정하지 않는다.
- 폴더의 기초 자료(원본 데이터, 기존 코드)는 삭제하거나 수정하지 않는다.
- 참고용 동작 샘플: model/sklearn_sample, pytorch_sample, tensorflow_sample
